...
- **全ページのヘッダ左端**に、ドキュメント一覧（`http://127.0.0.1:30002/` = サーバのトップ）へ戻る
  `← ドキュメント一覧` リンクを置く（`docs-home` クラス、`href="/"`）。これはテンプレートに組み込み済みで、
  トップページ（`index.html`）・各章ページの両方に自動で入る。ここでの「トップページ」は当該ドキュメント内の
  目次であり、`ドキュメント一覧` は全ドキュメントを束ねるサーバのトップページを指す（別物）。
