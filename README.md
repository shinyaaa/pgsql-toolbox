...
fast-forward できる場合はブランチごと fast-forward する。
fast-forward できない場合 (ローカルコミットで分岐している、
フィーチャーブランチや detached HEAD のまま、など) は HEAD を動かさず、
上記 3 ディレクトリのファイルだけを origin の内容に更新する。
ローカルでコミット済み・変更中のドキュメントは上書きされずスキップされ、
件数がボタン横に表示される。
インデックス (staging area) は変更しないため、更新されたファイルが
次のコミットに紛れ込むことはない。
